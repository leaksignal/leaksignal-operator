use crate::{envoy_json::OwnerInfo, Error};
...
impl LeaksignalIstioSpec {
    ...
    pub fn on_error(leak: Arc<LeaksignalIstio>, error: &Error, _context: Arc<Client>) -> Action {
...
impl ClusterLeaksignalIstioSpec {
    ...
    pub fn on_error(
        leak: Arc<ClusterLeaksignalIstio>,
        error: &Error,
        _context: Arc<Client>,
    ) -> Action {
    ...
    T::DynamicType: Default,
{
    ...
    if !is_deleted {
        ...
        let path =
            crate::proxy_mgr::check_or_add_proxy(&values.proxy_hash, values.native, &proxy_url)
                .await?;
